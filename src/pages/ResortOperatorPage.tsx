...
function buildBrief(snapshot: Awaited<ReturnType<typeof loadSnapshot>>) {
  ...
  dirtyRooms.forEach((unit: any) => {
    if (!snapshot.housekeeping.some((task: any) => task.unit_name === unit.unit_name)) actions.push({
      action_type: 'create_housekeeping_task', title: `Create cleaning task for ${unit.unit_name}`,
      description: 'Room is marked for cleaning but has no active housekeeping order.', target_table: 'units', target_id: unit.id,
      payload: { unit_name: unit.unit_name, room_type_id: unit.room_type_id || null, status: 'pending_inspection', cleaning_notes: 'Created by KAPWA Resort Operator after management approval.' }, risk_level: 'low',
    });
  });
  urgentRequests.forEach((request: any) => actions.push({
    action_type: 'escalate_guest_request', title: `Escalate guest request${request.guest_name ? ` from ${request.guest_name}` : ''}`,
    description: request.details || request.request_type || 'Urgent guest request requires staff attention.', target_table: 'guest_requests', target_id: request.id,
    payload: { status: 'escalated', updated_at: new Date().toISOString() }, risk_level: 'medium',
  }));
  snapshot.disputes.forEach((dispute: any) => actions.push({
    action_type: 'review_bill_dispute', title: 'Review open bill dispute', description: dispute.reason || dispute.notes || 'An unresolved billing dispute needs management review.',
    target_table: 'bill_disputes', target_id: dispute.id, payload: {}, risk_level: 'high',
  }));
...
export default function ResortOperatorPage() {
  ...
  const actionsQuery = useQuery({ queryKey: ['agent-actions'], queryFn: async () => { const { data, error } = await from('agent_actions').select('*').order('created_at', { ascending: false }).limit(50); if (error) throw error; return (data || []) as AgentAction[]; } });
  ...
  const runOperator = useMutation({
    mutationFn: async () => {
      ...
      if (adminToken) {
        ...
        toast.warning('AI runtime token is not set. Using deterministic operations analysis.');
      }

      const { data: run, error: runError } = await from('agent_runs').insert({ agent_type: 'resort_operator', trigger_type: 'manual', requested_by: 'admin', prompt: question, status: 'completed', summary: analysis, model_provider: provider, model_name: model, input_snapshot: snapshotQuery.data, output: { deterministic_summary: brief.summary, analysis, metrics: brief.metrics, proposed_actions: brief.actions }, completed_at: new Date().toISOString() }).select('*').single();
    ...
    onSuccess: result => { setLastAnswer(result.analysis); setRuntimeLabel(result.provider && result.model ? `${result.provider} · ${result.model}` : 'Deterministic fallback'); qc.invalidateQueries({ queryKey: ['agent-actions'] }); toast.success(`${result.actions.length} controlled action proposal${result.actions.length === 1 ? '' : 's'} created`); },
  ...
  const decideAction = async (action: AgentAction, decision: 'approved' | 'rejected') => {
    ...
    if (decision === 'rejected') { qc.invalidateQueries({ queryKey: ['agent-actions'] }); toast.success('Action rejected'); return; }
    if (!['create_housekeeping_task', 'escalate_guest_request'].includes(action.action_type)) { qc.invalidateQueries({ queryKey: ['agent-actions'] }); toast.success('Action approved for manual management execution'); return; }
    ...
    } finally { setExecutingId(null); qc.invalidateQueries({ queryKey: ['agent-actions'] }); }
  };
  ...
  return <div className="min-h-screen bg-background text-foreground p-4 md:p-8"><div className="max-w-6xl mx-auto space-y-6">
    ...
    <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 flex gap-3 text-sm"><ShieldAlert className="w-5 h-5 text-amber-600 shrink-0" /><p>Only housekeeping creation and urgent guest-request escalation can execute automatically after approval. Bookings, billing, payments, refunds, external messages, pricing, and deletions remain manual.</p></div>
  </div></div>;
}
